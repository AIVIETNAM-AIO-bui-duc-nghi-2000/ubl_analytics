...
# ==========================================
MONGO_URI = os.getenv("MONGO_URI", "mongodb://35.247.155.208:27017/")
PROXY_URL = os.getenv("HTTP_PROXY", None)  # e.g. "http://user:pass@proxy.com:8080"
DB_NAME = "countly"
...
async def build_queue_from_events(db):
    ...
    await db.scrape_queue.create_index(
        [("status", 1), ("next_retry", 1), ("retry_count", 1)]
    )
    ...
    for coll in group_1_collections:
        ...
        await db[coll].create_index(
            [("product_id", ASCENDING), ("current_url", ASCENDING)], background=True
        )
        await db[coll].create_index(
            [("viewing_product_id", ASCENDING)], background=True
        )
    ...
    await db["product_view_all_recommend_clicked"].create_index(
        [("viewing_product_id", ASCENDING), ("referrer_url", ASCENDING)],
        background=True,
    )
    ...
    try:
        await (
            db["product_view_all_recommend_clicked"].aggregate(pipeline_2).to_list(None)
        )
    except Exception as e:
...
def extract_product_data(
    html_text: str, source_url: str, product_id: str
) -> Optional[dict]:
                ...
                for item in items:
                    ...
                    if isinstance(item, dict):
                        if item.get("@type") == "Product":
                            ...
                            item.get("@type") == "ItemPage"
                            and isinstance(item.get("mainEntity"), dict)
                            and item["mainEntity"].get("@type") == "Product"
                        ):
                            product_node = item["mainEntity"]
                    ...
                    if product_node:
                        ...
                        raw_data = {
                            ...
                            "url": product_node.get("url", source_url),
...
async def crawler_worker(task_queue: asyncio.Queue, result_queue: asyncio.Queue):
    """Worker fetching data using curl_cffi to bypass Akamai WAF."""
    global consecutive_fails

    session_requests = 0
    current_profile = random.choice(BROWSER_PROFILES)
    proxies = {"http": PROXY_URL, "https": PROXY_URL} if PROXY_URL else None
    session = AsyncSession(impersonate=current_profile, proxies=proxies)
    ...
    while not shutdown_event.is_set():
        ...
        url = task["url"]
        if url.startswith("/"):
            url = f"https://www.glamira.com{url}"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if consecutive_fails >= CIRCUIT_BREAKER_LIMIT:
                    logging.critical(
                        "CRITICAL: Akamai blocked our IP completely. Stopping the system!"
                    )
                    shutdown_event.set()
            ...
            await result_queue.put(
                {"status": "NETWORK_ERROR", "task": task, "error": str(e)}
            )
        ...
        while not shutdown_event.is_set() or not result_queue.empty():
            try:
                ...
                if status == "SUCCESS":
                    ...
                    batch_ops.append(
                        UpdateOne(
                            {"_id": task["_id"]}, {"$set": {"status": "COMPLETED"}}
                        )
                    )
                    ...
                    batch_ops.append(
                        UpdateOne(
                            {"_id": task["_id"]}, {"$set": {"status": "DEAD_LINK"}}
                        )
                    )
                        ...
                        UpdateOne(
                            {"_id": task["_id"]},
                            {
                                "$set": {
                                    ...
                                    "last_error": res.get("error", status),
                                }
                            },
                        )
                    )
                ...
                if len(batch_ops) >= 50:
                    ...
                    logging.info(
                        f"Progress: {processed_count}/{total_tasks} ({percent:.1f}%) | Success: {success_count} | Fail: {fail_count}"
                    )
...
async def main():
    ...
    client = AsyncIOMotorClient(
        MONGO_URI, maxPoolSize=50, minPoolSize=10, serverSelectionTimeoutMS=5000
    )
    db = client[DB_NAME]
    ...
    workers = [
        asyncio.create_task(crawler_worker(task_queue, result_queue))
    ...
    while not shutdown_event.is_set():
        ...
        cursor = (
            db.scrape_queue.find(
                {"status": "PENDING", "next_retry": {"$lte": time.time()}}
            )
            .sort([("retry_count", 1), ("next_retry", 1)])
            .limit(50)
        )
        ...
        if not tasks:
            ...
            processing_count = await db.scrape_queue.count_documents(
                {"status": "PROCESSING"}
            )
            pending_count = await db.scrape_queue.count_documents({"status": "PENDING"})
        ...
        for t in tasks:
            ...
            await db.scrape_queue.update_one(
                {"_id": t["_id"]}, {"$set": {"status": "PROCESSING"}}
            )
